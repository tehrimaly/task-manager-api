...
def test_create_returns_unique_ids():
    t1 = create_task("Task One")
    t2 = create_task("Task Two")
    assert t1["id"] != t2["id"]
...
def test_create_non_string_title_raises():
    with pytest.raises(ValueError):
        create_task(123)
...
def test_get_returns_correct_task():
    create_task("Task A")
    t2 = create_task("Task B")
    assert get_task(t2["id"])["title"] == "Task B"
...
def test_update_priority():
    task = create_task("Walk", priority="low")
    updated = update_task(task["id"], priority="high")
    assert updated["priority"] == "high"
 
 
def test_update_description():
    task = create_task("Cook")
    updated = update_task(task["id"], description="Make pasta")
    assert updated["description"] == "Make pasta"
...
def test_list_filter_incomplete():
    t1 = create_task("Done")
    create_task("Pending")
    update_task(t1["id"], completed=True)
    result = list_tasks(filter_completed=False)
    assert len(result) == 1
